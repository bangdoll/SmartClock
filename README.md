...
- ✨ **Minimal (極簡風)**：純淨大字，使用纖細字重
- 🔥 **Nixie (擬輝光管)**：模擬 EleksMaker 風格，蜂巢網格背景搭配橙黃到深紅的漸層發光
...
### � 車用視覺優化 (Car Mode)
專為車載螢幕與手機支架設計的顯示模式：
- **螢幕恆亮 (Wake Lock)**：網頁開啟時自動防止螢幕休眠，適合長時間行車使用。
- **雙擊全螢幕 (Fullscreen)**：在畫面任意處 **快速點兩下**，即可隱藏瀏覽器網址列，切換至沈浸式滿版顯示。
...
### Q: 使用 AirPlay 鏡像時，過幾分鐘會自動斷線或變黑？
這通常是手機省電設定導致的。請嘗試以下解決方案：
1. **關閉自動鎖定**：前往 iPhone「設定」>「螢幕顯示與亮度」>「自動鎖定」，暫時改為「永不」。
2. **關閉低耗電模式**：確保電池圖示不是黃色的，低耗電模式會限制 Wi-Fi 傳輸導致斷線。
3. **檢查車機設定**：確認車機內的螢幕保護程式或休眠設定已關閉。
